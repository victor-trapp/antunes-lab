...
function fetchUser(id) {
    return new Promise(resolve => setTimeout(() => resolve({ id, name: id === 1 ? "Alice" : "Bob" }), 10));
}
function fetchScore(userId) {
    return new Promise(resolve => setTimeout(() => resolve(userId === 1 ? 95 : 80), 10));
}
function mayFail(succeed) {
    return new Promise((resolve, reject) => setTimeout(() => (succeed ? resolve("ok") : reject(new Error("oops"))), 10));
}
...
async function safeCall(succeed) {
    if (succeed) {
